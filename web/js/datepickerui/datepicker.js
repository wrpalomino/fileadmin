...
jQuery(document).ready(function() {
  ...
  {
    // modified by William, 12/05/2013: to avoid wrong date selection
    /*var d = new Date(
      $('#' + name + 'year').val(),
      $('#' + name + 'month').val() - 1,
      $('#' + name + 'day').val()
    );*/
    var year = ($('#' + name + 'year').val() != '') ? $('#' + name + 'year').val() : (new Date).getFullYear();
    var month = ($('#' + name + 'month').val() != '') ? $('#' + name + 'month').val()-1 : (new Date).getMonth();
    var day = ($('#' + name + 'day').val() != '') ? $('#' + name + 'day').val() : '1'; // not every month has 31
    var d = new Date(year, month, day);
    //alert(year+'/'+month+'/'+day);
    
    $('#datepick_' + name).datepicker('option', 'defaultDate', d);
    
    $('#datepick_' + name).datepicker('option', 'changeYear', d);
    $('#datepick_' + name).datepicker('option', 'changeMonth', d);
    $('#datepick_' + name).datepicker('option', 'yearRange', "-120:+10", d);
  ...
  $('select[id$=year]').each(function(i) {
    ...
    if ($(this).parents('div.sf_admin_filter').length == 0 && !$(this).hasClass('noauto'))
    {
      ...
      {
        // modified by William, 28/01/2013: do not set today as default for date dropboxes, just for datepicker
        /*var today = new Date();
    ...
    var basepath = (location.host == "localhost") ? "localhost/fileadmin" : location.host;

    // datepicker!
    $('<input style="width:0;height:0;border:0" id="datepick_' + name + '">').insertAfter(this).datepicker({
      buttonImage: 'http://'+basepath+'/js/datepickerui/images/calendar.png',
