...
abstract class ahBaseFormDoctrine extends sfFormDoctrine
{
  ...
  public function embedRelations(array $relations)
  {
    ...
    foreach ($relations as $relationName => $relationSettings)
    {
      ...
      else
      {
        ...
        // added by William, 04/05/2012
        $formArgs[0]['first'] = true;
        ...
        foreach ($this->getObject()->$relationName as $index => $childObject)
        {
          ...
          // added by William, 04/05/2012
          if ($formArgs[0]['first']) $formArgs[0]['first'] = false;
        ...
        // added by William, 02/05/2012: to set a different format for the container form
        $custom_decorator = new EmbeddedFormContainerSchemaFormatter($subForm->getWidgetSchema());
        $subForm->widgetSchema->addFormFormatter('MyCFormatterEFC', $custom_decorator);
        $subForm->widgetSchema->setFormFormatterName('MyCFormatterEFC');
  ...
  public function saveEmbeddedForms($con = null, $forms = null)
  {
    ...
    /*foreach ($this->embeddedForms['FileCourtDates'] as $k => $v) {
      echo '===='.$k .'=>'. $v->getName().'<br/>';
      foreach ($v as $k1 => $v1) {
        echo '========='.$k1 .'=>'. $v1->getName().'<br/>';
        foreach ($v1 as $k2 => $v2) {
          echo '================'.$k2 .'=>'. $v2->getName().'<br/>';
        }
      }
    }*/
