...
class sfWidgetFormDependentSelect extends sfWidgetForm
{
    ...
    protected function generateJavascriptVar($baseName, $var = null)
    {
        ...
        // added by William, 17/02/2012: to fix bug about embeded forms
        $var1 = ($var) ? sfInflector::underscore($var) : null;
        if (strpos($jsVar, $var1) !== false) {
          $arr = explode('[', str_replace(']', '', $baseName));
          
          if ( (isset($arr[2])) && ($arr[2]) && (!is_numeric($arr[2])) ) {  // this is an embedded form (if index is numeric could be an array)
            //echo 'fixing select depended in embedded form<br/>';
            $jsVar = str_replace(array('_'.$arr[1], $var1), array('', $this->getOption('master_widget')), $jsVar);
          }
        }
        //echo $baseName.' >>>> jsvar: '.$jsVar.'<br/>';
